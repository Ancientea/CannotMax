...
from src.game.login import LoginManager
...
def get_recognizer():
    ...
    if _recognizer is None:
        ...
        from src.recognition.recognize import RecognizeMonster
...
class DeviceInstance:
    def __init__(self, port):
        ...
        self.connector = load_data.AdbConnector(self.serial)
...
<<<<<<< HEAD
            logger.info(
                f"[{self.serial}] 开始启动实例，游戏模式: {game_mode}, 自动投资: {is_invest}"
            )
=======
            # 保存设置
...
>>>>>>> origin/main
            ...
            login_success = self.login_manager.auto_login_with_restart(
                first_start=True,
                stop_callback=lambda: not self.stop_event.is_set(),
            )
            ...
            self.auto_fetch = auto_fetch.AutoFetch(
                ...
                field_recognizer=(
                    get_field_recognizer() if FIELD_FEATURE_COUNT > 0 else None
                ),
...
>>>>>>> origin/main
    ...
    def start_all(self):
        ...
        logger.info(
            f"开始启动多开实例，端口列表: {ports}, 游戏模式: {game_mode}, 自动投资: {is_invest}"
        )
    ...
    def package_data(self):
        try:
            ...
            if zip_filename and Path(zip_filename).exists():
                # 在文件浏览器中高亮显示文件
                subprocess.run(
                    f'explorer /select,"{Path(zip_filename).absolute()}"'
                )
    ...
    def _state_to_chinese(state):
        from src.game.auto_fetch import GameState
    ...
    def _create_port_widget(self, port):
        ...
        toggle_btn.clicked.connect(
            lambda checked, p=port: self._toggle_port(p)
        )
    ...
    def update_display(self):
        ...
        for port in input_ports:
            ...
            if port in self.instances:
                ...
                is_running = (
                    instance.auto_fetch
                    and instance.auto_fetch.auto_fetch_running
                )
                ...
                if is_running:
                    ...
                    elapsed = (
                        time.time() - af.start_time if af.start_time else 0
                    )
                    hours, remainder = divmod(elapsed, 3600)
                    minutes, _ = divmod(remainder, 60)
                    ...
                    widgets["toggle_btn"].setText(
                        "停止" if is_starting else "启动"
                    )
            else:
