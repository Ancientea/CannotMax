...
class StateMachine:
    ...
    def transition_to(self, new_state):
        ...
        if new_state in allowed_transitions[self.state]:
            ...
            logger.error(
                f"Illegal state transition from {self.state} to {new_state}"
            )
...
class SandboxSimulator:
    def __init__(self, master: tk.Tk, battle_data):
        ...
        self.selected_monster_for_drag = (
            None  # 当前拖动的怪物对象 (Monster 类型)
        )
    ...
    def update_ui_state(self):
        ...
        self.deploy_button.config(
            state=states["deploy"]["state"], text=states["deploy"]["text"]
        )
        self.confirm_start_button.config(
            state=states["confirm_start"]["state"]
        )
        self.pause_button.config(
            state=states["pause"]["state"], text=states["pause"]["text"]
        )
    ...
    def on_mouse_drag(self, event):
        if self.selected_monster_for_drag and self.state_machine.state in [
            ...
            self.selected_monster_for_drag.target_deployment_position = (
                FastVector(new_grid_x, new_grid_y)
            )
    ...
    def create_widgets(self):
        ...
        tk.Button(speed_frame, text="应用", command=self.apply_speed).pack(
            side=tk.LEFT
        )

        self.deploy_button = tk.Button(
            top_control_frame, text="部署怪物", command=self.enter_setup_phase
        )
        self.deploy_button.pack(side=tk.LEFT, padx=5)

        self.confirm_start_button = tk.Button(
            top_control_frame,
            text="开始战斗",
            command=self.start_actual_simulation,
            state=tk.DISABLED,
        )
        self.confirm_start_button.pack(side=tk.LEFT, padx=5)

        self.clear_button = tk.Button(
            top_control_frame, text="清空战场", command=self.clear_sandbox
        )
        ...
        self.restore_button = tk.Button(
            top_control_frame,
            text="恢复站位",
            command=self.restore_initial_positions,
            state=tk.DISABLED,
        )
    ...
    def show_message_below_button(
        self, message, is_error=False, duration=5000
    ):
    ...
    def draw_grid(self):
        ...
        if self.battle_field and self.battle_field.danger_zone_size() > 0:
            danger_zone = min(
                self.battle_field.danger_zone_size(), self.grid_height / 2 + 1
            )
            ...
            self.canvas.create_rectangle(
                self.canvas_width,
                0,
                self.canvas_width - (danger_zone + 1) * self.cell_size,
                self.canvas_height,
                fill="#cccc00",
                outline="",
            )
            self.canvas.create_rectangle(
                ...
                self.canvas_height,
                0,
                self.canvas_height - danger_zone * self.cell_size,
                fill="#cccc00",
                outline="",
            )
        ...
        for i in range(self.grid_width + 1):
            ...
            if i == 1:  # 红色列的右边缘线
                self.canvas.create_line(
                    x, 0, x, self.canvas_height, fill=soft_red_line
                )
            elif i == self.grid_width - 1:  # 蓝色列的左边缘线
                self.canvas.create_line(
                    x, 0, x, self.canvas_height, fill=soft_blue_line
                )
            else:
                self.canvas.create_line(
                    x, 0, x, self.canvas_height, fill="lightgray"
                )  # 垂直网格线
        ...
        for i in range(self.grid_height + 1):
            ...
            if self.cell_size > 0:  # 确保 cell_size > 0 避免问题
                ...
                self.canvas.create_line(
                    0, y, self.cell_size, y, fill="#E07070"
                )  # 示例：红色区域内的水平线
                # 在蓝色区域内绘制颜色稍浅的水平线
                self.canvas.create_line(
                    self.canvas_width - self.cell_size,
                ...
                self.canvas.create_line(
                    0, y, self.canvas_width, y, fill="lightgray"
                )
        ...
        if self.state_machine.state in [AppState.SIMULATING, AppState.PAUSED]:
            for monster_obj in self.battle_field.monsters:
                if monster_obj.is_alive and monster_obj.target is not None:
                    self.canvas.create_line(
                        monster_obj.position.x * self.cell_size,
                        ...
                        fill=(
                            "#FF3030"
                            if monster_obj.faction == Faction.LEFT
                            else "#3030FF"
                        ),
                        width=1,
                        arrow="last",
    ...
    def draw_unit(self, unit, monster: "Monster"):
        ...
        self.canvas.create_image(
            x_pixel, y_pixel, image=icon_to_draw, tags=("unit",)
        )
        ...
        health_ratio = (
            min(1, unit.health / unit.max_health) if unit.max_health > 0 else 0
        )
        current_health_width = max(0, bar_width * health_ratio)
        self.canvas.create_rectangle(
            x_pixel - bar_width / 2,
            health_bar_y - bar_height / 2,
            x_pixel + bar_width / 2,
            health_bar_y + bar_height / 2,
            fill="#400000",
            outline="",
        ...
        if current_health_width > 0:
            self.canvas.create_rectangle(
                x_pixel - bar_width / 2,
                health_bar_y - bar_height / 2,
                x_pixel - bar_width / 2 + current_health_width,
                health_bar_y + bar_height / 2,
                fill="#FF3030",
                outline="",
            )
        ...
        skill_ratio = (
            min(1, unit.skill / unit.max_skill) if unit.max_skill > 0 else 0
        )
        current_skill_width = bar_width * skill_ratio
        self.canvas.create_rectangle(
            x_pixel - bar_width / 2,
            skill_bar_y - bar_height / 2,
            x_pixel + bar_width / 2,
            skill_bar_y + bar_height / 2,
            fill="black",
            outline="",
        ...
        if current_skill_width > 0:
            self.canvas.create_rectangle(
                x_pixel - bar_width / 2,
                skill_bar_y - bar_height / 2,
                x_pixel - bar_width / 2 + current_skill_width,
                skill_bar_y + bar_height / 2,
                fill=attack_state_color,
                outline="",
            )
    ...
    def on_mouse_down(self, event):
        ...
        if self.state_machine.state in [AppState.PAUSED, AppState.SETUP]:
            ...
            for unit_in_list in reversed(self.units):
                monster = self.battle_field.get_monster_with_id(
                    unit_in_list.monster_global_id
                )
                if not monster:
                    continue
                ...
                if (
                    abs(event.x - monster_center_x_px) < icon_half_size_px
                    and abs(event.y - monster_center_y_px) < icon_half_size_px
                ):
    ...
    def restore_initial_positions(self):
        ...
        self.battle_field.gameTime = (
            self.initial_battlefield.gameTime
        )  # 保持时间连续性
...
def main():
    ...
    initial_battle_setup = {
        "left": {"Vvan": 4, "炮击组长": 3, "“庞贝”": 2},
        "right": {"大喷蛛": 6, "冰爆源石虫": 23},
        "result": "left",
    }
    ...
    try:
        ...
        logger.error(
            "错误: 无法解析 stdin 中的 JSON 数据，请检查格式，将使用默认配置。"
        )

    except Exception as e:
        logger.error(
            f"从 stdin 读取或解析时发生未知错误: {e}，将使用默认配置。"
        )
    ...
    if problematic_monsters_found:
        error_messages.append(
            "警告: " + "； ".join(problematic_monsters_found)
        )
    ...
    if error_messages:
        ...
        if "monsters.json 未找到或为空" in " | ".join(error_messages):
            logger.error(
                "由于 monsters.json 缺失或错误，模拟器可能无法正常工作。"
            )
