...
class HistoryMatch:
    ...
    def __init__(self, csv_path="data/arknights.csv"):
        # 初始化时加载历史对局数据
    ...
    def load_history_data(self):
        ...
        try:
            ...
            if df.shape[1] >= total_features + 1:  # 至少包含特征和结果列
                ...
                right_monster_end = (
                    MONSTER_COUNT + FIELD_FEATURE_COUNT + MONSTER_COUNT
                )
                right_field_end = (
                    MONSTER_COUNT
                    + FIELD_FEATURE_COUNT
                ...
                # 分别提取怪物和地形特征
                left_monsters = df.iloc[:, 0:left_monster_end].values.astype(
                    float
                )
                ...
                # 兼容旧格式：只有怪物特征
                self.past_left = df.iloc[:, 0:MONSTER_COUNT].values.astype(
                    float
                )
    ...
    def render_similar_matches(
        ...
        # 计算需求索引处的数量差和
        diff_L = np.sum(
            np.abs(Lh[:, need_L_idx] - cur_left[need_L_idx]), axis=1
        )
        diff_R = np.sum(
            np.abs(Rh[:, need_R_idx] - cur_right[need_R_idx]), axis=1
        )

        # 计算对手兵种在本方需求中的命中数，取最小值作为 match_other
        hit_L = np.sum(
            hist_pres_Rh[:, need_L_idx] & pres_L[need_L_idx], axis=1
        )
        hit_R = np.sum(
            hist_pres_Lh[:, need_R_idx] & pres_R[need_R_idx], axis=1
        )
        ...
        if need_L_idx.size > 0:
            ratios_L = Lh[:, need_L_idx] / np.maximum(
                cur_left[need_L_idx], 1e-12
            )
        ...
        if need_R_idx.size > 0:
            ratios_R = Rh[:, need_R_idx] / np.maximum(
                cur_right[need_R_idx], 1e-12
            )
        ...
        labs = np.where(
            swap[top5],
            np.where(self.labels[top5] == "L", "R", "L"),
            self.labels[top5],
        )
        tgtL = (
            need_L_idx[np.argmax(cur_left[need_L_idx])]
            if need_L_idx.size
            else None
        )
        tgtR = (
            need_R_idx[np.argmax(cur_right[need_R_idx])]
            if need_R_idx.size
            else None
        )
        ...
        lw = np.sum(
            [
                ...
                == (
                    "L"
                    if (Lh[i, tgtL] if tgtL is not None else 0) > 0
                    else "R"
                )
        ...
        rw = np.sum(
            [
                ...
                == (
                    "L"
                    if (Lh[i, tgtR] if tgtR is not None else 0) > 0
                    else "R"
                )
                for i, lab in zip(top5, labs)
            ]
    ...
    def get_terrain_names(self, idx, is_swapped=False):
        ...
        # 根据是否镜像选择地形特征
        terrain_features = (
            self.past_right_terrain[idx]
            if is_swapped
            else self.past_left_terrain[idx]
        )
        ...
        try:
            from src.recognition.field_recognition import FieldRecognizer
            ...
            if field_recognizer.is_ready():
                ...
                return (
                    "+".join(active_terrains) if active_terrains else "无地形"
                )
        except Exception:
            pass
