...
class SandboxSimulator:
    ...
    def create_widgets(self):
        ...
        # ====== 蒙特卡洛控制区 ======
        mc_frame = tk.LabelFrame(
            self.master, text="蒙特卡洛模拟", padx=10, pady=8
        )
        ...
        self.mc_canvas.grid(
            row=2, column=0, columnspan=6, sticky="we", pady=(6, 2)
        )
        self.mc_canvas.bind(
        ...
        self.mc_canvas = tk.Canvas(mc_frame, height=160, bg="white")
        self.mc_canvas.grid(
            row=2, column=0, columnspan=5, sticky="we", pady=(6, 2)
        )
        self.canvas = None
    ...
    def _render_mc_text_and_chart(
        self, counts: Counter, done: int, target: int
    ):
    ...
    def draw_mc_barchart(self, items, total):
        ...
        for i, (label, cnt) in enumerate(items):
            ...
            pct = f"{(cnt/total):.1%}"
            c.create_text(
                (x0 + x1) // 2, y0 - 10, text=f"{cnt} ({pct})", anchor="s"
            )
            # 类别名
            c.create_text(
                (x0 + x1) // 2, H - pad_y + 12, text=label, anchor="n"
            )
