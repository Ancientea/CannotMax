...
class ArknightsApp:
    def __init__(self, root):
        ...
        # 创建按钮
        btn_style = {"bg": "#555555", "fg": self.FG_COLOR, "activebackground": "#777777", "activeforeground": self.FG_COLOR}
        self.next_button = tk.Button(root, text="下一个", command=self.show_next_row, **btn_style)
        self.next_button.pack(side=tk.RIGHT, padx=10, pady=10)

        self.prev_button = tk.Button(root, text="上一个", command=self.show_prev_row, **btn_style)
        self.prev_button.pack(side=tk.RIGHT, padx=10, pady=10)

        self.delete_button = tk.Button(root, text="删除数据", command=self.delete_current_row, **btn_style)
        self.delete_button.pack(side=tk.RIGHT, padx=10, pady=10)

        # 添加行号显示和跳转功能
        self.row_label = tk.Label(root, text="当前行号: 0", bg=self.BG_COLOR, fg=self.FG_COLOR)
        self.row_label.pack(side=tk.LEFT, padx=10)

        self.row_entry = tk.Entry(root, width=5, bg="#555555", fg=self.FG_COLOR, insertbackground=self.FG_COLOR)
        self.row_entry.pack(side=tk.LEFT, padx=5)

        self.jump_button = tk.Button(root, text="跳转", command=self.jump_to_row, **btn_style)
    ...
    def show_row(self, row_index):
            ...
            try:
                ...
                if value > 0:  # 仅显示非0值
                    if self.images.get(str(i)):
                        tk.Label(self.top_frame, image=self.images[str(i)], bg=self.BG_COLOR).grid(row=0, column=i - 1, padx=2)
                    else:
                        tk.Label(self.top_frame, text=f"ID:{i}", font=("Arial", 12), bg=self.BG_COLOR, fg=self.FG_COLOR).grid(row=0, column=i - 1, padx=2)
                    tk.Label(self.top_frame, text=str(int(value)), font=("Arial", 16, "bold"), bg=self.BG_COLOR, fg=self.FG_COLOR).grid(row=1, column=i - 1)
            except ValueError:
                print(f"Skipping invalid value: {row[i - 1]} at column {i - 1}")

        # 插入空白间隔
        gap_column = self.MONSTER_COUNT - 1  # 间隔列索引
        tk.Label(self.top_frame, text="", bg=self.BG_COLOR).grid(row=0, column=gap_column, padx=50)  # 添加水平间距
            ...
            try:
                ...
                if value > 0:  # 仅显示非0值
                    ...
                    if self.images.get(img_idx):
                        tk.Label(self.top_frame, image=self.images[img_idx], bg=self.BG_COLOR).grid(row=0, column=i - 1, padx=2)
                    else:
                        tk.Label(self.top_frame, text=f"ID:{img_idx}", font=("Arial", 12), bg=self.BG_COLOR, fg=self.FG_COLOR).grid(row=0, column=i - 1, padx=2)
                    tk.Label(self.top_frame, text=str(int(value)), font=("Arial", 16, "bold"), bg=self.BG_COLOR, fg=self.FG_COLOR).grid(row=1, column=i - 1)
            except ValueError:
                print(f"Skipping invalid value: {row[i - 1]} at column {i - 1}")
        ...
        if orig_path:
            ...
            right = w * 0.8
            bottom = h
            cropped_image = orig_image.crop((int(left), int(top), int(right), int(bottom)))
            ...
            tk.Label(self.bottom_frame, image=cropped_image_tk, bg=self.BG_COLOR).pack(side=tk.TOP, pady=5)
            self.bottom_frame.images.append(cropped_image_tk)
        else:
            tk.Label(self.bottom_frame, text=f"找不到原图: {img_name}", bg=self.BG_COLOR, fg=self.FG_COLOR).pack(side=tk.TOP, pady=5)

        # 2. 显示 L/R 文字和色条（中间）
        result_text = row[self.MONSTER_COUNT * 2].strip()
        tk.Label(self.bottom_frame, text=result_text, font=("Arial", 20, "bold"), bg=self.BG_COLOR, fg=self.FG_COLOR).pack(side=tk.TOP, pady=2)
        
        # 添加色彩长条 Canvas
        bar_width = 200
        bar_height = 15
        color_bar = tk.Canvas(self.bottom_frame, width=bar_width, height=bar_height, highlightthickness=0, bg=self.BG_COLOR)
        color_bar.pack(side=tk.TOP, pady=5)
        
        if result_text.upper() == 'L':
            color_bar.create_rectangle(0, 0, bar_width // 2, bar_height, fill="yellow", outline="")
            color_bar.create_rectangle(bar_width // 2, 0, bar_width, bar_height, fill="gray", outline="")
        elif result_text.upper() == 'R':
            color_bar.create_rectangle(0, 0, bar_width // 2, bar_height, fill="gray", outline="")
            color_bar.create_rectangle(bar_width // 2, 0, bar_width, bar_height, fill="yellow", outline="")
        else:
            # 默认颜色（异常数据时）
            color_bar.create_rectangle(0, 0, bar_width, bar_height, fill="gray", outline="")
        ...
        if res_path:
            ...
            tk.Label(self.bottom_frame, image=res_image_tk, bg=self.BG_COLOR).pack(side=tk.TOP, pady=5)
            self.bottom_frame.images.append(res_image_tk)
        else:
            tk.Label(self.bottom_frame, text=f"找不到结果图", bg=self.BG_COLOR, fg=self.FG_COLOR).pack(side=tk.TOP, pady=5)
