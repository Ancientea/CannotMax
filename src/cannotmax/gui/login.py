...
from cannotmax.config.paths import LOGIN_IMAGES_DIR, PROCESS_IMAGES_DIR
...
class LoginManager:
    ...
    def __init__(self, connector, max_restart_count=3):
        ...
        self.template_dir = LOGIN_IMAGES_DIR
    ...
    def match_template(self, screenshot, template_name, threshold=0.9):
        ...
        for scale in np.linspace(0.5, 1.5, 10):
            ...
            if (
                resized.shape[0] > screenshot_gray.shape[0]
                or resized.shape[1] > screenshot_gray.shape[1]
            ):
                break
    ...
    def restart_game(self):
        ...
        is_adb = (
            hasattr(self.connector, "device_serial") and self.connector.device_serial
        )
        ...
        try:
            if is_pc:
                ...
                subprocess.run(
                    f"{adb_path} -s {self.connector.device_serial} shell am force-stop com.hypergryph.arknights",
                    shell=True,
                )
                self._log(logging.INFO, "关闭游戏进程成功")
        except Exception as e:
        ...
        try:
            if is_pc:
                ...
                adb_path = getattr(self.connector, "adb_path", "adb")
                subprocess.run(
                    f"{adb_path} -s {self.connector.device_serial} shell am start -n com.hypergryph.arknights/com.u8.sdk.U8UnityContext"
                )
    ...
    def auto_login(self, first_start=False, stop_callback=None):
        ...
        # 确保连接器已连接
        if (
            not hasattr(self.connector, "is_connected")
            or not self.connector.is_connected
        ):
        ...
        if first_start:
            ...
            if screenshot is not None:
                ...
                matched, _ = self.match_template(
                    screenshot, "competition_page", threshold=0.7
                )
                if matched:
                ...
                try:
                    ...
                    for template_name in ["0", "1"]:
                        ...
                        template_path = PROCESS_IMAGES_DIR / f"{template_name}.png"
                        if template_path.exists():
                            ...
                            if template is not None:
                                ...
                                if max_val > 0.7:
                                    logger.info(
                                        f"已在争锋频道页面，找到模板 {template_name}.png"
                                    )
                                    return True
                except Exception as e:
                    ...
                    if template_path.exists():
                        try:
                            ...
                            if template is not None:
                                ...
                                if max_val > 0.7:
                                    logger.info(
                                        f"已在战斗前准备阶段，找到模板 {template_name}.png，无需登录"
                                    )
                                    return True
                        except Exception as e:
                            logger.debug(
                                f"检测战斗前准备模板 {template_name}.png 失败: {e}"
                            )
        ...
        while time.time() - start_time < 40:
            ...
            matched, pos = self.match_template(
                screenshot, "login_button", threshold=0.9
            )
            if matched:
                rel_x = pos[0] / w
                rel_y = pos[1] / h
                self._log(
                    logging.INFO,
                    f"登录按钮位置: ({pos[0]}, {pos[1]}), 相对坐标: ({rel_x:.2f}, {rel_y:.2f})",
                )
        ...
        while time.time() - start_time < 70:
            ...
            if screenshot is not None:
                ...
                ann_matched, _ = self.match_template(
                    screenshot, "announcement_close", threshold=0.9
                )
                event_matched, _ = self.match_template(
                    screenshot, "event_claim_close", threshold=0.9
                )
                ...
                if comp_matched or ann_matched or event_matched:
                    ...
                    self._log(
                        logging.INFO,
                        f"检测到游戏界面已加载 (耗时: {elapsed}秒)，提前结束等待",
                    )
            ...
            if screenshot is not None:
                for button in close_buttons:
                    ...
                    if matched:
                        ...
                        self.connector.click((rel_x, rel_y))
                        self._log(
                            logging.INFO, f"第 {i + 1} 次检测，关闭 {button} 页面"
                        )
    ...
    def _check_and_click_competition_page(
        self, screenshot, sleep_with_check, check_stop
    ):
        """争锋频道入口检测和点击方法"""
        matched, pos = self.match_template(
            screenshot, "competition_page", threshold=0.7
        ...
        if matched:
            ...
            self._log(
                logging.INFO,
                f"争锋频道入口位置: ({pos[0]}, {pos[1]}), 相对坐标: ({rel_x:.2f}, {rel_y:.2f})",
            )
            self.connector.click((rel_x, rel_y))
    ...
    def restart_and_login(self, first_start=False, stop_callback=None):
        ...
        self._log(
            logging.INFO,
            f"尝试重启游戏 (第 {self.restart_count + 1}/{self.max_restart_count} 次)",
        )
    ...
    def auto_login_with_restart(self, first_start=False, stop_callback=None):
        ...
        for _ in range(self.max_restart_count - 1):
            if not self.restart_and_login(
                first_start=False, stop_callback=stop_callback
            ):
