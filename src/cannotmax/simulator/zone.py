...
class PoisonZone(EffectZone):
    ...
    def apply_effect(self, target):
        ...
        target.status_system.apply(
            BuffEffect(
                type=BuffType.POWER_STONE, duration=VIRTUAL_TIME_DELTA * 2, source=self
            )
        )
    ...
    def contains(self, target) -> bool:
        ...
        if self.battle_field.danger_zone_size() > 0:
            ...
            if (
                target.position.x < size + 1
                or target.position.x > self.battle_field.map_size[0] - size - 1
            ) or (
                target.position.y < size
                or target.position.y > self.battle_field.map_size[1] - size
            ):
                return True
        return False
...
class WineZone(EffectZone):
    ...
    def apply_effect(self, target):
        # 添加或更新持续伤害效果
        target.status_system.apply(
            BuffEffect(type=BuffType.WINE, duration=VIRTUAL_TIME_DELTA * 2, source=self)
        )
    ...
    def contains(self, target) -> bool:
        ...
        ).magnitude <= self.radius and target.faction == self.faction
