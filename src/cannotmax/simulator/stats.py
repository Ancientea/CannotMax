...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(
        f"{'怪物名称':<10}\t{'偏差方向':<8}\t{'偏差分数':<8}\t{'p值':<10}\t{'样本量'}"
    )
    print("-" * 60)
    for name, row in result_df.iterrows():
        direction = "低估" if row["bias_score"] > 0 else "高估"
        print(
            f"{name:<10}\t{direction:<8}\t{row['bias_score']:.2f}{'*' if row['p_value'] < 0.01 else '':<4}"
...
sorted_monsters = sorted(
    err_counter.items(), key=lambda x: (-x[1] / counter[x[0]], x[0])
)
...
for monster, count in sorted_monsters:
    ...
    err_total = count
    print(
        f"{monster}: {err_total}/{total} = {err_total / total} 估计值:【{str}】{-power_counter[monster] / total * 100}"
    )
