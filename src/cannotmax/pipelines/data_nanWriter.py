...
from cannotmax.models.transformer import UnitAwareTransformer
...
class SafetyChecker:
    ...
    def _process_row(self, row):
        ...
        left_count = np.clip(
            np.abs(features[:midpoint]), 0, CONFIG["max_feature_value"]
        )
        ...
        # 转换为张量并添加batch维度
        return (
            torch.tensor(left_sign, dtype=torch.float32)
            .unsqueeze(0)
            .to(CONFIG["device"]),
            torch.tensor(left_count, dtype=torch.float32)
            .unsqueeze(0)
            .to(CONFIG["device"]),
            torch.tensor(right_sign, dtype=torch.float32)
            .unsqueeze(0)
            .to(CONFIG["device"]),
            torch.tensor(right_count, dtype=torch.float32)
            .unsqueeze(0)
            .to(CONFIG["device"]),
        )
    ...
    def check_csv(self):
        ...
        with tqdm(total=total_rows, desc="Checking rows") as pbar:
            for chunk in pd.read_csv(
                CONFIG["csv_path"], header=None, chunksize=chunk_size
            ):
                normal_chunk = []
                ...
                if normal_chunk:
                    pd.DataFrame(normal_chunk).to_csv(
                        self.clean_csv_path, mode="a", header=False, index=False
                    )
        ...
        for row in problematic_rows:
            print(
                f"行号 {row[0] if isinstance(row, tuple) else row}: ",
