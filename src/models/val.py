...
from src.models.train import UnitAwareTransformer, ArknightsDataset
...
def main():
    ...
    try:
        model = torch.load(
            str(MODELS_DIR / "best_model_full.pth"),
            map_location=device,
            weights_only=False,
        )
    except TypeError:  # 如果旧版本 PyTorch 不认识 weights_only
        model = torch.load(str(MODELS_DIR / "best_model_full.pth"), map_location=device)
    model.eval()
