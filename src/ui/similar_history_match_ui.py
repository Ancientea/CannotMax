...
from src.analysis.similar_history_match import HistoryMatch
from src.core.config import MONSTER_COUNT, MONSTER_DATA
    ...
    def render_similar_matches(self, left_monsters, right_monsters):
        try:
            ...
            for idx in top_indices:
                self.add_history_match(
                    idx, sims[idx], left_monsters, right_monsters
                )

        except Exception as e:
            logger.error(f"渲染历史对局失败: {str(e)}")

    def add_history_match(
        self, idx, similarity, left_monsters, right_monsters
    ):
        """添加单个历史对局到面板"""
        # 获取历史数据
        ...
        # 判断是否需要镜像历史对局
        should_swap = (
            len(setL_cur ^ setR_past) + len(setR_cur ^ setL_past)
    ...
    def create_team_widget(self, side, counts, is_winner):
        ...
        for i, count in enumerate(counts):
            if count > 0:
                ...
                try:
                    pixmap = QPixmap(
                        f"src/resources/assets/images/{MONSTER_DATA['原始名称'][i+1]}.png"
                    )
