...
from src.core.paths import simulation_path, PROJECT_ROOT, MODELS_DIR
...
def clean_data(
    ...
    if rows_to_remove:
        cleaned_data = pd.concat(
            [cleaned_data, pd.DataFrame([last_row] * len(rows_to_remove))],
            ignore_index=True,
        )
    cleaned_data = cleaned_data.drop_duplicates(
        subset=cleaned_data.columns[:-3], keep="first"
    ).reset_index(drop=True)
    features_cleaned, labels_cleaned, pic_names_cleaned = (
        cleaned_data.iloc[:, :-3],
        cleaned_data.iloc[:, -3],
        cleaned_data.iloc[:, -2],
    )
    ...
    for idx, pic_name in enumerate(pic_names_cleaned):
        ...
        try:
            detected_full_names = set(
                predict_scene(None, idx_to_class, image_path, threshold=0.5)
            )
            row_image_data = {}
            for condensed_name, full_names in grouped_elements.items():
                num_positions = len(full_names)
                if num_positions == 1:
                    row_image_data[condensed_name] = (
    ...
    if features_cleaned.shape[1] != 122:
        print(
            f"警告: 期望122个原始特征，但检测到{features_cleaned.shape[1]}个。将按前61列和剩余列进行分割。"
        )
    ...
    headers_elements_L = [
        f"{i}L" for i in range(62, 62 + num_element_features)
    ]
    ...
    headers_elements_R = [
        f"{i}R"
        for i in range(
            num_r_features + 1, num_r_features + 1 + num_element_features
        )
    ]
...
if __name__ == "__main__":
    ...
    model_dir = MODELS_DIR / "battlefield_recognize"
    ...
    if not os.path.exists(pth_model_path):
        ...
        print(
            f"请确保存在与ONNX同名的PTH文件: {os.path.basename(pth_model_path)}"
        )
    else:
        clean_data(
            input_file,
