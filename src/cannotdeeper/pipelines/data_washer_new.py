...
def remove_duplicate_subsequences_easy(listdata, threshold=3):
    ...
    for i in range(len(listdata) - threshold - 1):
        if is_continuous_sublist(
            listdata[i + 1 : i + threshold + 2], listdata[: i + threshold + 1]
        ):
...
def read_and_remove_zeros(filename, MONSTER_NUM=56):
    ...
    for i in range(np_array.shape[0]):
        if np.all(np_array[i][MONSTER_NUM:] == 0) or np.all(
            np_array[i][:MONSTER_NUM] == 0
        ):
            all_zeros.append(i)
...
def view_monster_counts(listdata):
    ...
    num_right = [
        list(map(int, map(float, i[MONSTER_NUM : MONSTER_NUM * 2]))) for i in listdata
    ]
    ...
    processed_data2 = [
        processed_data[j] for j in range(len(processed_data)) if j not in mwdata
    ]
...
def del_duplicate_by_time(listdata, delete_no_time=True):
    ...
    if not delete_no_time:
        ...
        data_with_time_ok = [
            ...
            if j not in second_deleted_original
        ]
        wrong_time = second_deleted_original
    else:
        data_with_time_ok = [
            data_with_time[j] for j in range(len(data_with_time)) if j not in wrong_time
        ]
    ...
    for c in csvlist:
        ...
        with open(c, "r") as file:
            ...
            for row in csv_reader:
                if (
                    isfloat(row[0])
                    and "" not in row[:lines_num]
                    and len(row) > lines_num
                ):
                    numbers = list(
                        map(int, map(float, row[:lines_num]))
                    )  # 转换为整数列表
                    vals = [row[lines_num]]
...
def is_list_true(onelist):
    ...
    mincostL = sum(
        [
            (left[i] - 1) * cost_list[i][0]
            + (left[i] - 1) * (left[i] - 2) * cost_list[i][1] / 2
            + 0.01
            for i in range(len(left))
            if (cost_list[i] != [-1, -1]) and (left[i] > 0)
        ]
    )
    maxcostL = sum(
        [
            (left[i] + 1) * cost_list[i][0]
            + left[i] * (left[i] + 1) * cost_list[i][1] / 2
            - 0.01
            for i in range(len(left))
            if (cost_list[i] != [-1, -1]) and (left[i] > 0)
        ]
    ...
    mincostR = sum(
        [
            (right[i] - 1) * cost_list[i][0]
            + (right[i] - 1) * (right[i] - 2) * cost_list[i][1] / 2
            + 0.01
            for i in range(len(right))
            if (cost_list[i] != [-1, -1]) and (right[i] > 0)
        ]
    )
    maxcostR = sum(
        [
            (right[i] + 1) * cost_list[i][0]
            + right[i] * (right[i] + 1) * cost_list[i][1] / 2
            - 0.01
            for i in range(len(right))
            if (cost_list[i] != [-1, -1]) and (right[i] > 0)
        ]
    ...
    for i in range(len(round_cost_list)):
        if max(mincostL, round_cost_list[i][0]) <= min(
            maxcostL, round_cost_list[i][1]
        ) and max(mincostR, round_cost_list[i][0]) <= min(
            maxcostR, round_cost_list[i][1]
        ):
            roundlist.append(i)
    if roundlist != []:
        return True
...
def recognize_review(data, img_floder, matched_threshold=0.1, ocr_threshold=0.5):
    ...
    for idx, row in tqdm.tqdm(enumerate(data), total=len(data), desc="Processing rows"):
        ...
        try:
            ...
            results = recognize.process_regions(
                main_roi, img, matched_threshold, ocr_threshold
            )
            # 处理结果
            for res in results:
                if "error" in res:
                    print(
                        f"识别失败 行号: {idx}, 图片: {img_name}, 错误类型: {res['error']}",
                        file=sys.stderr,
                    )
                    break
                if res["matched_id"]:
                    if res["region_id"] < 3:
                        ...
                        ref_row[res["matched_id"] - 1 + MONSTER_NUM] = int(
                            res["number"]
                        )
            else:
                # 检查数据行是否与参考行匹配
...
def process_full(
    ...
    if re_recognize_imgs:
        newdata, deleted6 = recognize_review(
            newdata, img_floder, matched_threshold, ocr_threshold
        )
    deleted7 = []
    if open_black_list:
        ...
        if i != []:
            ...
            if flag == 0:
                wrong_type_list.append(
                    ["不合法的数据：", merge([i + 1 for i in deleted0])]
                )
                wrong_type_list.append(
                    ["重复出现的连续数据*：", merge([i + 1 for i in secori])]
                )
            elif flag == 1:
                wrong_type_list.append(
                    ["未包含时间轴的数据：", merge([i + 1 for i in secori])]
                )
            elif flag == 2:
                wrong_type_list.append(
                    ["时间轴信息重复的数据：", merge([i + 1 for i in secori])]
                )
            elif flag == 3:
                wrong_type_list.append(
                    ["怪物信息错误的数据：", merge([i + 1 for i in secori])]
                )
            elif flag == 4:
                wrong_type_list.append(
                    ["不符合出怪权重规则的数据：", merge([i + 1 for i in secori])]
                )
            elif flag == 5:
                wrong_type_list.append(
                    ["经图片识别错误的数据*：", merge([i + 1 for i in secori])]
                )
            elif flag == 6:
                wrong_type_list.append(
                    ["黑名单内数据：", merge([i + 1 for i in secori])]
                )
        flag += 1
...
def create_gui():
    ...
    folder_frame = ttk.LabelFrame(
        root, text="处理文件夹(处理文件夹及其所有子文件夹下的CSV文件，并合并为一个)"
    )
    folder_frame.grid(row=0, column=0, padx=10, pady=5, sticky="ew")

    # 处理文件夹的组件
    ttk.Label(folder_frame, text="选择文件夹:").grid(
    ...
    ttk.Label(folder_frame, text="中间保存文件（不进行最终去重）:").grid(
        row=1, column=0, padx=5, sticky="w"
    )
    interim_save = tk.StringVar()
    ttk.Entry(folder_frame, textvariable=interim_save, width=40).grid(
        row=1, column=1, padx=5
    )
    ttk.Button(
        folder_frame,
        text="浏览",
        command=lambda: interim_save.set(
    ...
    ttk.Label(folder_frame, text="最终保存文件:").grid(
        row=2, column=0, padx=5, sticky="w"
    )
    final_save = tk.StringVar()
    ttk.Entry(folder_frame, textvariable=final_save, width=40).grid(
        row=2, column=1, padx=5
    )
    ttk.Button(
        folder_frame,
        text="浏览",
        command=lambda: final_save.set(
            filedialog.asksaveasfilename(
                defaultextension=".csv",
                filetypes=[("CSV文件", "*.csv"), ("所有文件", "*.*")],
            )
    ...
    ttk.Checkbutton(
        folder_frame,
        text="不依赖时间戳清理重复子序列（在大数据集会非常慢，通常关闭）",
        variable=remove_dup,
    ).grid(row=3, column=0, columnspan=3, sticky="w")

    del_time = tk.BooleanVar(value=True)
    ttk.Checkbutton(folder_frame, text="删除无时间戳数据", variable=del_time).grid(
    ...
    ttk.Checkbutton(
        folder_frame,
        text="启用图片二次识别（必须指定图片路径）",
        variable=re_recognize_var,
    ).grid(row=6, column=0, padx=5, sticky="w")

    # 添加匹配阈值设置
    ttk.Label(folder_frame, text="匹配阈值:").grid(
        row=6, column=1, padx=(20, 5), sticky="e"
    )
    matched_threshold_var = tk.DoubleVar(value=0.1)
    ttk.Entry(folder_frame, textvariable=matched_threshold_var, width=6).grid(
        row=6, column=2, sticky="w"
    )

    # 添加OCR阈值设置
    ttk.Label(folder_frame, text="OCR阈值:").grid(
        row=6, column=3, padx=(20, 5), sticky="e"
    )
    ocr_threshold_var = tk.DoubleVar(value=0.5)
    ttk.Entry(folder_frame, textvariable=ocr_threshold_var, width=6).grid(
        row=6, column=4, sticky="w"
    )

    # 调整后续行号（原row=6改为row=7开始）
    ttk.Label(folder_frame, text="图片文件夹路径:").grid(
        row=7, column=0, padx=5, sticky="w"
    )
    img_folder_path = tk.StringVar()
    ttk.Entry(folder_frame, textvariable=img_folder_path, width=40).grid(
        row=7, column=1, padx=5
    )
    ttk.Button(
        folder_frame,
        text="浏览",
        command=lambda: img_folder_path.set(filedialog.askdirectory()),
    ...
    ttk.Entry(file_frame, textvariable=file_path, width=40).grid(
    ...
    ttk.Entry(file_frame, textvariable=save_path, width=40).grid(
        row=1, column=1, padx=5
    )
    ttk.Button(
        file_frame,
        text="浏览",
        command=lambda: save_path.set(
            filedialog.asksaveasfilename(
                defaultextension=".csv",
                filetypes=[("CSV文件", "*.csv"), ("所有文件", "*.*")],
            )
    ...
    ttk.Checkbutton(
        file_frame,
        text="不依赖时间戳清理重复子序列（在大数据集会非常慢，通常关闭）",
        variable=remove_dup_file,
    ).grid(row=2, column=0, columnspan=3, sticky="w")

    del_time_file = tk.BooleanVar(value=True)
    ttk.Checkbutton(file_frame, text="删除无时间戳数据", variable=del_time_file).grid(
    ...
    ttk.Checkbutton(
        file_frame,
        text="启用图片二次识别（必须指定图片路径）",
        variable=re_recognize_file_var,
    ).grid(row=5, column=0, padx=5, sticky="w")

    # 匹配阈值
    ttk.Label(file_frame, text="匹配阈值:").grid(
        row=5, column=1, padx=(20, 5), sticky="e"
    )
    matched_threshold_file_var = tk.DoubleVar(value=0.1)
    ttk.Entry(file_frame, textvariable=matched_threshold_file_var, width=6).grid(
        row=5, column=2, sticky="w"
    )

    # OCR阈值
    ttk.Label(file_frame, text="OCR阈值:").grid(
        row=5, column=3, padx=(20, 5), sticky="e"
    )
    ocr_threshold_file_var = tk.DoubleVar(value=0.5)
    ttk.Entry(file_frame, textvariable=ocr_threshold_file_var, width=6).grid(
        row=5, column=4, sticky="w"
    )

    # 调整后续行号
    ttk.Label(file_frame, text="图片文件夹路径:").grid(
        row=6, column=0, padx=5, sticky="w"
    )
    img_folder_file_path = tk.StringVar()
    ttk.Entry(file_frame, textvariable=img_folder_file_path, width=40).grid(
        row=6, column=1, padx=5
    )
    ttk.Button(
        file_frame,
        text="浏览",
        command=lambda: img_folder_file_path.set(filedialog.askdirectory()),
    ...
    def process_folder_wrapper():
        ...
        thread = ProcessingThread(
            ...
            args=(
                ...
                del_time.get(),
                open_black.get(),
                re_recognize_var.get(),
                Path(img_folder_path.get()),
                matched_threshold_var.get(),
                ocr_threshold_var.get(),
            ),
            callback=callback,
        )
        thread.start()
    ...
    def process_file_wrapper():
        ...
        thread = ProcessingThread(
            ...
            args=(
                ...
                del_time_file.get(),
                open_black_file.get(),
                re_recognize_file_var.get(),
                Path(img_folder_file_path.get()),
                matched_threshold_file_var.get(),
                ocr_threshold_file_var.get(),
            ),
            callback=callback,
        )
        thread.start()
