...
def find_big(crop, x_ratio, minR, maxR, width, p1=30, p2=35):
    ...
    for subimage, i in zip(crop, x_ratio):
        ...
        if circles_big is not None:
            ...
            logger.warning(
                f"section{x_ratio.index(i)} big circle not detected"
            )
...
def find_small(crop_small, x_ratio_small, x_ratio, minR, maxR, width):
    ...
    for subimage, i in zip(crop_small, x_ratio_small):
        ...
        if circles_small is not None:
            ...
            circles_small_bias[:, 0] = circles_small[0][:, 0] + np.round(
                i * width
            )
            ...
            logger.warning(
                f"section{x_ratio.index(i)} small circle not detected"
            )
...
def detect_outliers(coords, threshold=0.1):
    ...
    # 找出距离均值超过阈值倍标准差的点
    outliers = np.where(
        avg_distances > mean_avg_distance + threshold * std_avg_distance
    )[0]
    ...
    if high_tol == False:
        ...
        nums = np.round(
            np.array(
                [
                    [
                        cx + (nums_bias + 0 * k) * r - r * t,
                        cy + r * t,
                        cx + (nums_bias_inn + 2 * k) * r + r * t,
                        nums_y,
                    ],
                    [
                        cx + (nums_bias + 2 * k) * r - r * t,
                        nums_y,
                        cx + (nums_bias_inn + 4 * k) * r + r * t,
                        cy + r * t,
                    ],
                    [
                        cx + (nums_bias + 4 * k) * r - r * t,
                        cy + r * t,
                        cx + (nums_bias_inn + 6 * k) * r + r * t,
                        nums_y,
                    ],
                    [
                        cx + (-nums_bias_inn + 6 * k + m) * r - r * t,
                        cy + r * t,
                        cx + (-nums_bias + 8 * k + m) * r + r * t,
                        nums_y,
                    ],
                    [
                        cx + (-nums_bias_inn + 8 * k + m) * r - r * t,
                        nums_y,
                        cx + (-nums_bias + 10 * k + m) * r + r * t,
                        cy + r * t,
                    ],
                    [
                        cx + (-nums_bias_inn + 10 * k + m) * r - r * t,
                        cy + r * t,
                        cx + (-nums_bias + 12 * k + m) * r + r * t,
                        nums_y,
                    ],
                ]
            )
...
def filter(results_big, results_small, height):
    ...
    if small_key == 0:
        if big_key == 1:
            ...
            r_refer = (
                np.abs(filtered_small[1, 0] - filtered_small[0, 0]) / 16.39
            )  # 大圆的参考半径
            ...
            for x, y, radius, n in filtered_big:
                if n in [0, 1, 2]:
                    ...
                    p_cx.append(
                        x - ((2 * n + 1) * k * r_refer + 4.710 * r_refer)
                    )
            ...
            while std_x > 0.5 * r_refer:
                ...
                if filtered_big_p.size == 0:
                    logger.error(results_big)
                    raise IndexError(
                        "std_x筛选出现问题，请检查以上数据输入是否合法"
                    )

                mean_value = np.mean(filtered_big_p[:, -1])
                abs_diff = np.abs(filtered_big_p[:, -1] - mean_value)
                outlier_index = np.argmax(abs_diff)
                filtered_big_p = np.delete(
                    filtered_big_p, outlier_index, axis=0
                )
                std_x = np.std(filtered_big_p[:, -1])
            ...
            if results_big.shape[0] <= 2:  # 大圆数量不够筛选
                ...
                for x, y, radius, n in results_big:
                    if n in [0, 1, 2]:
                        ...
                        a_cx = x - (
                            (2 * n + 1) * k * r_refer + 4.710 * r_refer
                        )
                    a_cy = y + radius
                    std.append([a_cx, a_cy])
                _, out_index = detect_outliers(
                    std, threshold=0.02 * np.mean(results_big[:, 2])
                )
...
def cutFrame(image, high_tol=False):
    ...
    try:
        crop_blur, crop_small, x_ratio, x_ratio_small = preprocess(
            image, blur=11
        )
        ...
        filtered_big, filtered_small, high_tol = filter(
            results_big, results_small, height
        )

    except IndexError:
        try:
            crop_blur, crop_small, x_ratio, x_ratio_small = preprocess(
                image, blur=7, spare=1
            )
            results_big = find_big(
                crop_blur,
                x_ratio,
    ...
    # 使用最小二乘法求解
    result = custom_least_squares(
        residuals, initial_guess, args=(filtered_big, filtered_small)
    )
    avatar, nums = create_frame(
        result.x[0], result.x[1], result.x[2], high_tol
    )
