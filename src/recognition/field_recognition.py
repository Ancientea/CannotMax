...
from src.core.paths import TOOLS_DIR, ensure_tmp_images_dir
...
class FieldRecognizer:
        ...
        with torch.no_grad():
            for location, boxes in ROI_COORDINATES.items():
                for i, box in enumerate(boxes):
                    ...
                    max_prob, predicted_index_tensor = torch.max(
                        probabilities, 0
                    )
    ...
    def recognize_field_elements(self, screenshot):
        ...
        try:
            ...
            # 保存临时文件用于识别
            temp_image_path = str(ensure_tmp_images_dir() / "temp_screenshot.png")
            ...
            logger.debug(
                f"场地识别完成，检测到元素: {list(detected_full_names)}"
            )
            return field_data
