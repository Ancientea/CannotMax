...
class RecognizeMonster:
    ...
    def process_regions(
        ...
        for idx, rel in enumerate(relative_regions):
            try:
                ...
                # 图像匹配
                matched_id, confidence = find_best_match(
                    sub_roi, self.ref_images
                )
                logger.info(
            ...
            try:
                ...
                processed = crop_to_min_bounding_rect(
                    processed
                )  # 去除多余黑框
                processed = add_black_border(
                    processed, border_size=3
                )  # 加上3像素黑框
    ...
    def do_num_ocr(self, img: cv2.typing.MatLike):
        result = self.rapidocr_eng(
            img, use_det=False, use_cls=False, use_rec=True
        )
        logger.info(
            f"OCR: text: '{result.txts[0]}', score: {result.scores[0]}"
        )
...
def load_ref_images(ref_dir="images"):
    ...
    for i in range(MONSTER_COUNT + 1):
        ...
        # 裁切模板匹配图像比例
        img_crop = img[
            int(img.shape[0] * 0.16) : int(
                img.shape[0] * 0.80
            ),  # 高度取靠上部分
            int(img.shape[1] * 0.18) : int(
                img.shape[1] * 0.82
            ),  # 宽度与高度一致
        ]
        # 调整参考图像大小以匹配目标图像
