...
import numpy as np
from src.core.paths import VENDOR_DIR
...
def resolve_maafw_path() -> str:
    ...
    candidates = [
        VENDOR_DIR / "bin" / "maafw",
        Path(sys.executable).parent / "vendor" / "bin" / "maafw",
        Path.cwd() / "vendor" / "bin" / "maafw",
    ]
    for p in candidates:
...
class MaaConnectionConfig:
    maa_binary_path: str = ""
    adb_path: str = r".\vendor\bin\platform-tools\adb.exe"
    device_serial: str = ""
...
class ConnectionTypeRegistry:
    _types: list[ConnectionType] = [
        ConnectionType(
            "adb", "ADB连接", "", "通用ADB连接，需手动指定设备地址"
        ),
        ConnectionType(
            "ldplayer", "雷电模拟器", "emulator-5554", "雷电模拟器默认ADB地址"
        ),
        ConnectionType(
            "mumu", "MuMu模拟器", "127.0.0.1:7555", "MuMu模拟器默认ADB地址"
        ),
        ConnectionType(
            "mumu12",
            "MuMu12模拟器",
        ...
        ConnectionType(
            "nox", "夜神模拟器", "127.0.0.1:62001", "夜神模拟器默认ADB地址"
        ),
    ]
...
class InputMethodRegistry:
    _methods: list[InputMethodOption] = [
        ...
        InputMethodOption(
            ...
            2,
            "MinitouchAndAdbKey",
            "minitouch注入+ADB按键，低延迟需root",
        ),
        InputMethodOption(
            "maatouch", 4, "Maatouch", "Maatouch注入，低延迟MAA自带"
        ),
        InputMethodOption(
            "emulator_extras",
            8,
...
class MaaAdbConnector:
    ...
    def connect(self):
        ...
        try:
            ...
            self._ctrl.set_screenshot_use_raw_size(
                self._config.screenshot_use_raw_size
            )
            ...
            if image is not None:
                ...
                self.screen_width, self.screen_height = (
                    self._get_window_size_fallback()
                )

            self.is_maa_available = True
            self.is_connected = True
            logger.info(
                f"MAA Framework ADB连接成功: {target_serial}, 分辨率: {self.screen_width}x{self.screen_height}"
            )
    ...
    def swipe(
        ...
        try:
            ...
            logger.info(
                f"MAA滑动: ({x1_coord},{y1_coord}) -> ({x2_coord},{y2_coord})"
            )
            self._ctrl.post_swipe(
                x1_coord, y1_coord, x2_coord, y2_coord, duration
            ).wait()
        except Exception as e:
...
class AdbConnectorAdapter:
    def __init__(self, adb_path: str = r".\vendor\bin\platform-tools\adb.exe"):
        from src.data import load_data

        self._legacy_connector = load_data.AdbConnector()
    ...
    def connect(self):
        if MaaFrameworkDetector.is_available():
            try:
                ...
                maa_connector.device_serial = (
                    self._legacy_connector.device_serial
                    or self._maa_config.device_serial
                )
