...
class LoginManager:
    ...
    def match_template(self, screenshot, template_name, threshold=0.9):
        ...
        for scale in np.linspace(0.5, 1.5, 10):
            ...
            result = cv2.matchTemplate(
                screenshot_gray, resized, cv2.TM_CCOEFF_NORMED
            )
        ...
        if found:
            ...
            h, w = int(template.shape[0] * scale), int(
            ...
            if max_val >= threshold:
                ...
                logger.debug(
                    f"未匹配到模板 {template_name}，最高置信度: {max_val}"
                )
                return False, (0, 0)
        else:
    ...
    def restart_game(self):
        ...
        is_adb = (
            hasattr(self.connector, "device_serial")
            and self.connector.device_serial
        )
                    ...
                    for i in range(winreg.QueryInfoKey(key)[0]):
                        ...
                        try:
                            ...
                            if "Arknights" in display_name:
                                ...
                                game_path = (
                                    Path(install_location) / "Arknights.exe"
                                )
                                if game_path.exists():
                        ...
                        # 如果从注册表获取失败，使用默认路径
                        game_path = Path(
                            "C:\\Program Files\\Arknights\\Arknights.exe"
                        )
                        if game_path.exists():
                            subprocess.Popen(str(game_path))
                            self._log(
                                logging.INFO,
                                f"使用默认路径启动游戏: {game_path}",
                            )
                        else:
    ...
    def restart_and_login(self, first_start=False, stop_callback=None):
        ...
        if self.restart_game():
            ...
            # 传递 stop_callback 给 auto_login
            if self.auto_login(
                first_start=first_start, stop_callback=stop_callback
            ):
                return True
            else:
    ...
    def auto_login_with_restart(self, first_start=False, stop_callback=None):
        ...
        self._log(
            logging.ERROR, f"已尝试 {self.max_restart_count} 次，登录失败"
        )
        return False
