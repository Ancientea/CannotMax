...
from src.core.paths import SIMULATION_DIR
...
def process_battle_data(csv_path):
    ...
    for _, row in df.iterrows():
        ...
        # 构建阵营字典（ID从1开始）
        left_army = {
            MONSTER_MAPPING[i]: int(count)
            for i, count in enumerate(left_data)
            if count > 0
        }
        right_army = {
            MONSTER_MAPPING[i]: int(count)
            for i, count in enumerate(right_data)
            if count > 0
        }
...
def main():
    """主函数"""
    # 加载怪物数据
    with open(SIMULATION_DIR / "monsters.json", encoding="utf-8") as f:
    ...
    battle_data = process_battle_data(SIMULATION_DIR / "arknights.csv")
    ...
    for scene_config in tqdm(battle_data):
        if VISUALIZATION_MODE:
            scene_config = {
                "left": {"宿主流浪者": 7, "污染躯壳": 14, "凋零萨卡兹": 5},
                "right": {"大喷蛛": 4, "杰斯顿·威廉姆斯": 1, "衣架": 10},
                "result": "right",
            }
        ...
        for i in range(3):
            ...
            if not battlefield.setup_battle(
                left_army, right_army, monster_data
            ):
                continue

            # 开始战斗
            if (
                battlefield.run_battle(visualize=VISUALIZATION_MODE)
                == Faction.LEFT
            ):
                leftWins += 1
            if leftWins >= 2:
        ...
        if (left_win and scene_config["result"] == "left") or (
            not left_win and scene_config["result"] == "right"
        ):
            win += 1
        else:
