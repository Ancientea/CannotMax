...
class Battlefield:
    ...
    def setup_battle(self, left_army, right_army, monster_data):
        ...
        for name, count in left_army.items():
            ...
            for _ in range(count):
                pos = FastVector(
                    random.uniform(0, 0.5), random.uniform(0, MAP_SIZE[1])
                )
                self.monster_temporal_area_left.append(
                    MonsterFactory.create_monster(
                        data, Faction.LEFT, pos, self
                    )
                )
        ...
        for name, count in right_army.items():
            ...
            for _ in range(count):
                ...
                self.monster_temporal_area_right.append(
                    MonsterFactory.create_monster(
                        data, Faction.RIGHT, pos, self
                    )
                )
    ...
    def check_victory(self):
        """检查胜利条件"""
        if self.current_spawn_left < len(
            self.monster_temporal_area_left
        ) or self.current_spawn_right < len(self.monster_temporal_area_right):
            return None
        alive_factions = set()
        for m in self.alive_monsters:
            if m.is_alive:
    ...
    def run_one_frame(self):
        ...
        if self.round < 40 or self.round > 90:
            if (
                self.current_spawn_left < len(self.monster_temporal_area_left)
                and self.round % 2 == 0
            ):
                self.append_monster(
                    self.monster_temporal_area_left[self.current_spawn_left]
                )
                self.current_spawn_left += 1

            if (
                self.current_spawn_right
                < len(self.monster_temporal_area_right)
                and self.round % 2 == 0
            ):
                self.append_monster(
                    self.monster_temporal_area_right[self.current_spawn_right]
                )
    ...
    def print_battlefield(self):
        ...
        for m in self.alive_monsters:
            if m.is_alive:
                x = np.minimum(
                    np.maximum(0, int(m.position.x * 2)), MAP_SIZE[0] * 2 - 1
                )
                y = np.minimum(
                    np.maximum(0, int(m.position.y * 2)), MAP_SIZE[1] * 2 - 1
                )
