...
def names(parts):
    return {name_lookup(p):c for p,c in parts if name_lookup(p)}

def test(n, label, left_parts, right_parts, expected, rounds=50):
    l = names(left_parts)
    r = names(right_parts)
    if not l or not r:
        with open(r'G:\314\CannotMax-main\data\test_result.txt','a',encoding='utf-8') as f:
            f.write(f"{n:>3} SKIP {label} - name not found\n")
        return
    wl=wr=0
    t0=time.time()
    ...
    match="OK" if (expected=="L" and left_rate>50) or (expected=="R" and left_rate<50) else "MIS"
    with open(r'G:\314\CannotMax-main\data\test_result.txt','a',encoding='utf-8') as f:
        f.write(f"{n:>3} {match} {label:<50} 预期{expected} 左{left_rate:.0f}% L{wl}/R{wr} [{time.time()-t0:.1f}s]" + (f" 崩{errors}" if errors else "") + "\n")

with open(r'G:\314\CannotMax-main\data\test_result.txt','w',encoding='utf-8') as f:
    f.write("="*80+"\n")
    f.write(f"{'编号':>3} {'状态':<4} {'对局':<50} 结果\n")
...
test(72, "暴走×3 vs 合声×6", [("暴走",3)],[("合声",6)], "R", 30)
